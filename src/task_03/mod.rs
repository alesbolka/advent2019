...
pub fn ex1 () {
    ...
    let wire1 = Wire::parse(&"R8,U5,L5,D3", &origin);
    let wire2 = Wire::parse(&"U7,R6,D4,L4", &origin);
    let crossings = Wire::find_intersects(&wire1, &wire2);

    assert!(Point::shortest_distance(&origin, &crossings) == 6);
}
...
pub fn ex2() {
    ...
    assert!(Point::shortest_distance(&origin, &crossings) == 159);
}
...
pub fn ex3() {
    ...
    assert!(Point::shortest_distance(&origin, &crossings) == 135);
}
